...
const ActivitiesView: React.FC<ActivitiesViewProps> = ({ 
  ...
  const handleDownloadTemplate = () => {
    const templateData = [
      {
        'Nama Ekskul': 'PRAMUKA',
        'Kategori (Wajib/Seni/Keagamaan/Olahraga/Teknologi)': 'Wajib',
        'Pembina/Mentor': 'Kak Seto',
        'Jadwal': 'Sabtu, 14:00 - 16:00',
        'ID Anggota (Pisahkan dengan koma)': students.slice(0, 3).map(s => s.id).join(', ')
      }
    ];

    const ws = XLSX.utils.json_to_sheet(templateData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Template Ekskul");
    XLSX.writeFile(wb, "Template_Ekskul.xlsx");
    onShowNotification("Template Excel berhasil diunduh!", "success");
  };

  const handleExport = () => {
    if (displayActivities.length === 0) {
      onShowNotification("Tidak ada data ekskul untuk diekspor.", "warning");
      return;
    }

    const exportData = displayActivities.map((act, idx) => ({
      'NO': idx + 1,
      'NAMA EKSKUL': act.name,
      'KATEGORI': act.category,
      'PEMBINA/MENTOR': act.coach || '-',
      'JADWAL': act.schedule || '-',
      'JUMLAH ANGGOTA': act.members ? act.members.length : 0
    }));

    const ws = XLSX.utils.json_to_sheet(exportData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Ekskul");
    XLSX.writeFile(wb, `Ekskul_Kelas_${classId}.xlsx`);
    onShowNotification("Data ekskul berhasil diekspor ke Excel!", "success");
  };
    ...
    reader.onload = (event) => {
      try {
        ...
        rawData.forEach((row: any) => {
          const name = row['Nama Ekskul'];
          const category = row['Kategori (Wajib/Seni/Keagamaan/Olahraga/Teknologi)'] || 'Wajib';
          const mentor = row['Pembina/Mentor'] || '';
          const schedule = row['Jadwal'] || '';
          const membersRaw = row['ID Anggota (Pisahkan dengan koma)'] || '';
          
          const members = membersRaw
            ? String(membersRaw).split(',').map((s: string) => s.trim()).filter((s: string) => students.some(st => st.id === s))
            : [];

          if (name && onAddExtracurricular) {
            onAddExtracurricular({
              id: `imported-ekskul-${Date.now()}-${Math.floor(Math.random() * 10000)}`,
              classId,
              name: String(name),
        ...
        if (importedCount > 0) {
          onShowNotification(`Berhasil mengimpor ${importedCount} ekskul!`, "success");
        } else {
          onShowNotification("Tidak ada ekskul yang valid atau fungsi tambah tidak tersedia.", "error");
        }
      } catch (err) {
        console.error(err);
