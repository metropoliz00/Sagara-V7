...
const BackupRestore: React.FC<BackupRestoreProps> = ({ data: initialData, onRestore }) => {
  ...
  const handleDownloadBackup = async () => {
    setIsBackingUp(true);
    try {
      // Use API to fetch ALL data from database, not just current client state
      const fullData = await apiService.backupData();
      
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
      const fileName = `SAGARA_FULL_BACKUP_${timestamp}.json`;
      ...
      showAlert("Backup database berhasil diunduh!", "success");
    } catch (err) {
      console.error(err);
      showAlert("Gagal mengunduh backup database. Silakan coba lagi.", "error");
    } finally {
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
            ...
            <div className="p-6">
                ...
                >
                    {isBackingUp ? (
                        <>
                            <Loader2 size={20} className="mr-2 animate-spin" /> Menyiapkan Backup...
                        </>
                    ) : (
                        <>
