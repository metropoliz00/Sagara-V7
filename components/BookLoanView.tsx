...
import { Book, Plus, Search, Trash2, CheckCircle, Clock, Filter, ChevronDown, ChevronUp, RotateCcw, Image as ImageIcon, Edit2, Save, X, Loader2, BookOpen } from 'lucide-react';
...
const BookLoanView: React.FC<BookLoanViewProps> = ({
  ...
  const [editDigitalUrlValue, setEditDigitalUrlValue] = useState<string>('');
  ...
  const handleUpdateStock = async (id: string) => {
    const updatedInventory = inventory.map(item => {
      if (item.id === id) {
        ...
        return { ...item, totalStock: editStockValue, stock: item.stock + diff, digitalUrl: editDigitalUrlValue };
      }
      return item;
    });
    ...
    if (!isDemoMode) {
      try {
        ...
        onShowNotification('Stok & URL buku berhasil diperbarui ke database!', 'success');
      } catch (error) {
  ...
  const startEditing = (book: BookInventory) => {
    ...
    setEditDigitalUrlValue(book.digitalUrl || '');
  };
              ...
              <div className="p-3">
                ...
                {editingBookId === book.id ? (
                  <div className="mt-2 flex flex-col space-y-1">
                    ...
                    <input 
                      type="text" 
                      value={editDigitalUrlValue}
                      onChange={(e) => setEditDigitalUrlValue(e.target.value)}
                      className="w-full px-2 py-1 text-xs border rounded"
                      placeholder="URL Buku Digital"
                    />
                  ...
                  <div className="mt-2 flex justify-between items-start">
                    <div>
                      ...
                      {book.digitalUrl && (
                        <div className="mt-1">
                          <button onClick={() => window.open(book.digitalUrl, '_blank')} className="flex items-center gap-1 text-[10px] bg-indigo-50 text-indigo-700 py-1 px-2 rounded hover:bg-indigo-100 transition-colors">
                            <BookOpen size={10} /> Baca
                          </button>
                        </div>
                      )}
                    </div>
