...
const GraduatesView: React.FC<GraduatesViewProps> = ({ onShowNotification, isReadOnly = false, onRestore }) => {
  ...
  const filteredGraduates = useMemo(() => {
    return graduates.filter(g => {
      ...
      const matchSearch = g.name.toLowerCase().includes(searchTerm.toLowerCase()) || 
                          ...
                          gNis.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const handleOpenModal = (graduate?: Graduate) => {
    if (graduate) {
      ...
      setFormData({
        ...graduate,
        nis: graduate.nis || '',
        sklUrl: graduate.sklUrl || '',
        isVisible: graduate.isVisible !== false
      });
    } else {
  ...
  const handleNisChange = async (nis: string) => {
    setFormData(prev => ({ ...prev, nis }));
    
    if (nis.length >= 3) {
      setLoadingNis(true);
      try {
        const student = await apiService.getStudentByNis(nis);
        if (student) {
          setFormData(prev => ({ 
            ...prev, 
            name: student.name,
  ...
  const handleToggleVisibility = async (graduate: Graduate) => {
    const updatedGraduate = {
      ...graduate,
      isVisible: graduate.isVisible === false ? true : false,
      updatedAt: Date.now()
    };
    try {
      await apiService.saveGraduate(updatedGraduate);
      setGraduates(prev => prev.map(g => g.id === graduate.id ? updatedGraduate : g));
      onShowNotification(
        updatedGraduate.isVisible 
          ? `Pengumuman untuk ${graduate.name} sekarang terlihat oleh siswa` 
          : `Pengumuman untuk ${graduate.name} sekarang disembunyikan dari siswa`, 
        "success"
      );
    } catch (e) {
      onShowNotification("Gagal mengubah visibilitas", "error");
    }
  };
  ...
  const handleExport = () => {
    const dataToExport = filteredGraduates.map((g, index) => ({
      ...
      'Link SKL/Ijazah': g.sklUrl || '',
      'Visibilitas': g.isVisible !== false ? 'Aktif' : 'Nonaktif'
    }));
  ...
  const handleDownloadTemplate = () => {
    const templateData = [{
      ...
      'Link SKL/Ijazah': 'https://example.com/skl-siswa.pdf',
      'Visibilitas': 'Aktif'
    }];
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const newGraduates: Graduate[] = data.map(row => {
          const vis = row['Visibilitas'] !== undefined 
            ? String(row['Visibilitas']).toLowerCase() === 'aktif' || String(row['Visibilitas']).toLowerCase() === 'true'
            : true;
          return {
            ...
            sklUrl: String(row['Link SKL/Ijazah'] || row['SKL/Ijazah URL'] || ''),
            isVisible: vis,
            createdAt: Date.now(),
            updatedAt: Date.now()
          };
    ...
    <div className="p-4 lg:p-8 max-w-7xl mx-auto h-full flex flex-col">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-auto custom-scrollbar">
          <table className="w-full min-w-max text-left border-collapse font-sans">
            <thead className="bg-slate-50 sticky top-0 z-10">
              <tr>
                <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-200 whitespace-nowrap">No</th>
                <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-200 whitespace-nowrap">NIS</th>
                <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-200 whitespace-nowrap">NISN</th>
                <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-200 whitespace-nowrap">Nama</th>
                <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-200 whitespace-nowrap">No. Ijazah</th>
                <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-200 whitespace-nowrap">SKL/Ijazah</th>
                <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-200 whitespace-nowrap">Status</th>
                <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-200 whitespace-nowrap">Visibilitas</th>
                <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-200 whitespace-nowrap">Tahun Lulus</th>
                <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-200 whitespace-nowrap">Melanjutkan Ke</th>
                {!isReadOnly && (
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-200 text-right whitespace-nowrap">Aksi</th>
                )}
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {filteredGraduates.length === 0 ? (
                <tr>
                  <td colSpan={isReadOnly ? 10 : 11} className="px-6 py-8 text-center text-slate-500">
                ...
                filteredGraduates.map((graduate, index) => (
                  <tr key={graduate.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4 text-sm text-slate-600 whitespace-nowrap">{index + 1}</td>
                    <td className="px-6 py-4 text-sm font-medium text-slate-800 whitespace-nowrap">{graduate.nis || '-'}</td>
                    <td className="px-6 py-4 text-sm font-medium text-slate-800 whitespace-nowrap">{graduate.nisn}</td>
                    <td className="px-6 py-4 text-sm font-semibold text-slate-800 uppercase whitespace-nowrap">{graduate.name.toUpperCase()}</td>
                    <td className="px-6 py-4 text-sm text-slate-600 whitespace-nowrap font-mono">{graduate.ijazahNumber || '-'}</td>
                    <td className="px-6 py-4 text-sm text-slate-600 whitespace-nowrap">
                      {graduate.sklUrl ? (
                        <a 
                          href={graduate.sklUrl} 
                          target="_blank" 
                          referrerPolicy="no-referrer"
                          rel="noopener noreferrer"
                          className="inline-flex items-center space-x-1 text-blue-600 hover:text-blue-800 font-medium bg-blue-50 px-2.5 py-1 rounded whitespace-nowrap border border-blue-100 transition-colors"
                        >
                          <Link size={14} />
                          <span>Buka Link</span>
                        </a>
                      ) : (
                        <span className="text-slate-400 italic text-xs whitespace-nowrap">Belum diisi</span>
                      )}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="px-2.5 py-1 text-xs font-semibold bg-emerald-100 text-emerald-700 rounded-full whitespace-nowrap">
                        {graduate.status}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2.5 py-1 text-xs font-semibold rounded-full whitespace-nowrap ${
                        graduate.isVisible !== false 
                          ? 'bg-blue-100 text-blue-700' 
                          : 'bg-slate-100 text-slate-600'
                      }`}>
                        {graduate.isVisible !== false ? 'Aktif' : 'Nonaktif'}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-sm text-slate-600 whitespace-nowrap font-medium">{graduate.graduationYear}</td>
                    <td className="px-6 py-4 text-sm text-slate-600 whitespace-nowrap">{graduate.continuedTo || '-'}</td>
                    {!isReadOnly && (
                      <td className="px-6 py-4 text-right whitespace-nowrap">
                        <div className="flex items-center justify-end space-x-2 whitespace-nowrap">
                          <button
                            onClick={() => handleToggleVisibility(graduate)}
                            className={`p-2 rounded-lg transition-colors ${
                              graduate.isVisible !== false 
                                ? 'text-[#5AB2FF] hover:bg-[#CAF4FF]/50' 
                                : 'text-slate-300 hover:bg-slate-100'
                            }`}
                            title={graduate.isVisible !== false ? "Sembunyikan dari Siswa" : "Tampilkan ke Siswa"}
                          >
                            {graduate.isVisible !== false ? <Eye size={18} /> : <EyeOff size={18} />}
                          </button>
                          <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Link SKL / Ijazah (URL)</label>
                <input
                  type="text"
                  value={formData.sklUrl || ''}
                  onChange={(e) => setFormData({ ...formData, sklUrl: e.target.value })}
                  className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-[#5AB2FF] focus:border-transparent outline-none transition-all font-mono text-xs"
                  placeholder="https://drive.google.com/..."
                />
              </div>
              <div className="flex items-center space-x-2 pt-2 bg-slate-50 p-3 rounded-xl border border-slate-100">
                <input
                  type="checkbox"
                  id="isVisible"
                  checked={formData.isVisible !== false}
                  onChange={(e) => setFormData({ ...formData, isVisible: e.target.checked })}
                  className="w-4 h-4 text-[#5AB2FF] border-slate-300 rounded focus:ring-[#5AB2FF]"
                />
                <label htmlFor="isVisible" className="text-sm font-semibold text-slate-700 select-none">
                  Tampilkan di portal akun siswa
                </label>
              </div>
            </div>
