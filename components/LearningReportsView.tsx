...
const CustomYAxisTick = (props: any) => {
  ...
  if (name.length <= 16) {
    return (
      <text x={x} y={y} dy={4} textAnchor="end" fill="#4b5563" fontSize={11} className="font-sans font-medium">
        {name}
      </text>
    );
  }
  ...
  if (commaIdx !== -1) {
    line1 = name.substring(0, commaIdx + 1).trim();
    line2 = name.substring(commaIdx + 1).trim();
  } else {
    const spaceIdx = name.indexOf(' ', 12);
    if (spaceIdx !== -1) {
      line1 = name.substring(0, spaceIdx).trim();
      line2 = name.substring(spaceIdx).trim();
    }
  }

  if (!line2) {
    return (
      <text x={x} y={y} dy={4} textAnchor="end" fill="#4b5563" fontSize={9} className="font-sans font-medium">
        {name}
      </text>
    );
  }
  ...
  return (
    <text x={x} y={y} textAnchor="end" fill="#4b5563" fontSize={9} className="font-sans font-medium">
...
const LearningReportsView: React.FC<LearningReportsViewProps> = ({ 
  reports, subjects, onSave, onDelete, classId, teachers = [], onNavigateToJournal, currentUser
}) => {
  ...
  const displayedTeachers = useMemo(() => {
    const list = [...teachers];
    if (currentUser?.role === 'guru' && currentUser?.fullName) {
      const exists = list.some(t => {
        const primaryName = (t.fullName || '').toLowerCase().trim();
        const curName = (currentUser.fullName || '').toLowerCase().trim();
        return primaryName === curName || String(t.id) === String(currentUser.id);
      });
      if (!exists) {
  ...
  const handleOpenModal = (report?: LearningReport) => {
    if (report) {
      ...
      setEditingReport({
        ...
        teacherName: currentUser?.fullName || ((teachers.length > 0 && teachers[0].fullName && teachers[0].fullName !== 'undefined') ? teachers[0].fullName : '')
      });
    }
    setIsModalOpen(true);
  };
                    ...
                    {recentActivity.map((report) => (
                        <div key={report.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group">
                            ...
                            <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-white p-4 rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                                {/* Header Row: Subject on left, Date on right */}
                                <div className="flex justify-between items-center mb-1 gap-2 min-w-0">
                                    <span className="font-bold text-gray-800 text-sm truncate flex-1 min-w-0">
                                        {report.subject} <span className="text-indigo-600 ml-1 shrink-0">(Kelas {report.classId})</span>
                                    </span>
                                    <span className="text-[10px] text-gray-400 shrink-0 whitespace-nowrap">{report.date}</span>
                                </div>
                                
                                {/* Teacher Name Row: Takes full width of the card and adjusts font size dynamically */}
                                <div className="mb-2 min-w-0">
                                    {(() => {
                                        const teacherName = (report.teacherName && report.teacherName !== 'undefined') ? report.teacherName : 'Guru';
                                        const nameLen = teacherName.length;
                                        const fontSize = nameLen > 30 ? 'text-[9px]' : nameLen > 24 ? 'text-[10px]' : nameLen > 18 ? 'text-[11px]' : 'text-xs';
                                        return (
                                            <span 
                                                className={`text-gray-500 font-semibold block whitespace-nowrap truncate ${fontSize}`} 
                                                title={teacherName}
                                            >
                                                {teacherName}
                                            </span>
