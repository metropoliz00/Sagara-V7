...
import { User, Lock, Loader2, ArrowRight, Sparkles, Eye, EyeOff, Code, X, Facebook, Instagram, MessageCircle, Music, MessageSquareText, Mail, Settings, AlertCircle, Youtube } from 'lucide-react';
...
const Login: React.FC<LoginProps> = ({ onLoginSuccess }) => {
  ...
  const [devInfo, setDevInfo] = useState<{ name: string; moto: string; photo: string; whatsapp?: string; facebook?: string; instagram?: string; tiktok?: string; youtube?: string; email?: string; } | null>({
      name: "Dedy Meyga Saputra, S.Pd, M.Pd",
      moto: "Mendedikasikan teknologi untuk kemudahan administrasi pendidikan Indonesia.",
      photo: "https://www.image2url.com/r2/default/images/1782534086979-5b315be8-d0b6-425a-8674-00f0fc9c3064.jpg",
      email: "metropoliz00@gmail.com",
      whatsapp: "https://wa.me/6285604431706",
    ...
    const fetchPublicInfo = async () => {
      if (apiService.isConfigured()) {
        try {
          ...
          if (profiles.school) {
            if (profiles.school.name && profiles.school.name.trim()) {
              setSchoolName(profiles.school.name);
            } else {
              setSchoolName('Dinas Pendidikan');
            }
            if (profiles.school.developerInfo) {
              const d = profiles.school.developerInfo;
              setDevInfo({
                name: d.name || PERMANENT_DEV_NAME,
                moto: d.moto || 'Mendedikasikan teknologi untuk kemudahan administrasi pendidikan Indonesia.',
                photo: d.photo || PERMANENT_DEV_PHOTO,
                email: d.email || 'metropoliz00@gmail.com',
                whatsapp: d.whatsapp || 'https://wa.me/6285604431706',
                facebook: d.facebook || 'https://www.facebook.com/share/18ivFCu4Fh/',
                instagram: d.instagram || 'https://www.instagram.com/dedy_meyga?igsh=cGp5ZTQ4bzFwZGp5',
                tiktok: d.tiktok || 'https://vt.tiktok.com/ZSXLjPNJQ/',
                youtube: d.youtube || 'https://youtube.com/@masgurupemula4766?si=bearyAkgJTr0eGkO'
              });
            }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!apiService.isConfigured() && username.toLowerCase() !== 'demo') {
      setError('Database belum terhubung. Silakan klik icon gerigi di kiri bawah untuk mengaktifkan database sekolah.');
      setLoading(false);
      return;
    }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-white relative overflow-hidden">
      ...
      {/* Settings FAB for Database Setup */}
      <div className="fixed bottom-6 left-6 z-20 flex items-center group">
        <button 
        ...
        {!localStorage.getItem('CUSTOM_SUPABASE_URL') && (
          <div className="ml-3 px-3 py-2 bg-gray-800 text-white text-xs rounded shadow-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none flex items-center gap-2">
            <AlertCircle size={14} className="text-yellow-400" />
            <span>Belum terhubung ke database. Klik untuk aktivasi.</span>
          </div>
        )}
      </div>
      ...
      {isSetupModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in" onClick={() => setIsSetupModalOpen(false)}>
          ...
          <div className="animate-fade-in-up">
            <div className="flex flex-col items-center mb-8">
               ...
               <div className="text-center">
                  ...
                  <p 
                    className="inline-block bg-[#CAF4FF] text-blue-800 font-bold uppercase tracking-wider px-4 py-2 rounded-full mt-4 border border-blue-200 shadow-sm whitespace-nowrap max-w-full overflow-hidden text-ellipsis"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in" onClick={() => setIsDevModalOpen(false)}>
            ...
            >
                <div className="p-8 flex flex-col items-center text-center relative">
                    ...
                    <img src={(devInfo && devInfo.photo) || PERMANENT_DEV_PHOTO} alt={(devInfo && devInfo.name) || PERMANENT_DEV_NAME} className="w-32 h-48 sm:w-40 sm:h-60 rounded-lg object-cover object-top mb-4 border-4 border-[#CAF4FF] shadow-lg"/>
                    <h3 className="text-sm sm:text-base md:text-lg font-bold text-gray-800 whitespace-nowrap w-full text-ellipsis overflow-hidden px-2">{(devInfo && devInfo.name) || PERMANENT_DEV_NAME}</h3>
                    <p className="text-gray-500 mt-2 text-xs md:text-sm italic mb-6">"{(devInfo && devInfo.moto) || 'Mendedikasikan teknologi untuk kemudahan administrasi pendidikan Indonesia.'}"</p>
                    
                    {/* Social Links / Layanan Pengaduan */}
                    {devInfo && (devInfo.whatsapp || devInfo.facebook || devInfo.instagram || devInfo.tiktok || devInfo.youtube || devInfo.email) && (
                        <div className="w-full pt-4 border-t border-gray-100">
                            ...
                            <div className="flex items-center justify-center gap-3">
                                ...
                                {devInfo.whatsapp && (
                                    <a href={devInfo.whatsapp.startsWith('http') ? devInfo.whatsapp : `https://wa.me/${devInfo.whatsapp}`} target="_blank" rel="noopener noreferrer" className="p-2 bg-green-50 text-green-600 hover:bg-green-100 rounded-full transition-colors" title="WhatsApp">
                                ...
                                {devInfo.youtube && (
                                    <a href={devInfo.youtube} target="_blank" rel="noopener noreferrer" className="p-2 bg-red-50 text-red-700 hover:bg-red-100 rounded-full transition-colors" title="YouTube">
                                        <Youtube size={18} />
                                    </a>
                                )}
                            </div>
