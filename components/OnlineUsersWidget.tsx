...
const OnlineUsersWidget: React.FC<OnlineUsersWidgetProps> = ({ currentUser, students = [], ttsEnabled = true, pathname }) => {
    ...
    channel
      ...
      .subscribe(async (status: string) => {
        if (status === 'SUBSCRIBED') {
          const studentRecord = currentUser.role === 'siswa' ? students.find(s => String(s.id) === String(currentUser.studentId)) : null;
          await channel.track({
            id: currentUser.id,
            name: currentUser.fullName,
            role: currentUser.role,
            photo: currentUser.photo || studentRecord?.photo || '',
            gender: currentUser.role === 'siswa' ? (studentRecord?.gender || 'L') : undefined,
  ...
  useEffect(() => {
    if (isOpen && !isDemo && supabase) {
      ...
      }, 60000); // Polling reduced to every 60s and only when active tab to save Disk IO
  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed ${positionClasses} right-6 z-40 bg-[#5AB2FF] text-white rounded-full w-14 h-14 shadow-lg hover:bg-[#4A9FE6] transition-all transform hover:-translate-y-1 active:scale-95 flex items-center justify-center group`}
                           ...
                           <div className="flex items-center gap-3">
                              <div className="relative">
                                <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center border-2 border-white shadow-sm overflow-hidden shrink-0">
                                    <img src={(user.role === 'siswa' || !user.photo) ? `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(user.name)}${user.gender === 'P' ? 'female' : 'male'}` : user.photo} alt="Avatar" className="w-full h-full object-cover" />
                                </div>
