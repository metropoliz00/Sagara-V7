...
const PiketTab: React.FC<PiketTabProps> = ({ piketGroups, students, onSave, onShowNotification }) => {
  const isSameDay = (d1: string | null, d2: string | null): boolean => {
    ...
    const norm1 = d1.trim().toLowerCase().replace(/['`]/g, "");
    const norm2 = d2.trim().toLowerCase().replace(/['`]/g, "");
    return norm1 === norm2;
  };
  ...
  const handleDrop = (e: React.DragEvent, targetDay: string | null) => {
    ...
    if (!targetDay) {
        if (sourceDay) {
            const sourceGroupIdx = newGroups.findIndex(g => isSameDay(g.day, sourceDay));
            if (sourceGroupIdx > -1) {
    ...
    const targetGroupIdx = newGroups.findIndex(g => isSameDay(g.day, targetDay));
    ...
    // 2. Add to target
    const newTargetGroupIdx = newGroups.findIndex(g => isSameDay(g.day, targetDay));
  ...
  const removeStudentFromDay = (day: string, studentId: string) => {
    let newGroups = [...localPiketGroups];
    const groupIndex = newGroups.findIndex(g => isSameDay(g.day, day));
    if (groupIndex > -1) {
